<?php 		$VM_ORDER_ID = $_POST['VM_ORDER_ID'];
			...
			$VM_ORDER_DETAIL_NAME = mysql_query("
				SELECT * 
				FROM VM_ORDER_DETAIL
				WHERE VM_ORDER_ID = '$VM_ORDER_ID' AND VM_TYPE = '$VM_TYPE' AND VM_NAME LIKE 'File Server %'
			")or die(mysql_error());
			...
			foreach ($VM_ORDER_DETAIL_NAME_ARRAY as $ID => $VM_ORDER_DETAIL_NAME_RECORD) {
				...
				if(is_numeric($LAST_NAME_IN_LIST[2])) {
					$NUM = $LAST_NAME_IN_LIST[2] + 1; 
					if($NUM > $HIGHEST_NAME_NUM) $HIGHEST_NAME_NUM = $NUM;
				}
			}
			if ($NUM_VM_ORDER_DETAIL_RESULTS>$HIGHEST_NAME_NUM) {
				$VM_NAME = "File Server " . $NUM_VM_ORDER_DETAIL_RESULTS;
			} else { 
				$VM_NAME = "File Server " . $HIGHEST_NAME_NUM;
			}
			...
			mysql_query("UPDATE VM_ORDER SET QTY_FILESERV ='$NUM_VM_ORDER_DETAIL_RESULTS' WHERE VM_ORDER_ID='$VM_ORDER_ID'");
			echo '	<form name="Next" action="index.php" method="post" style="display:inline; margin:0; padding:0;">
