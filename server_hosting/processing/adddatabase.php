<?php 			$VM_ORDER_ID = $_POST['VM_ORDER_ID'];
			...
			$VM_ORDER_DETAIL_NAME = mysql_query("
				SELECT * 
				FROM VM_ORDER_DETAIL
				WHERE VM_ORDER_ID = '$VM_ORDER_ID' AND VM_TYPE = '$VM_TYPE' AND VM_NAME LIKE 'Database %'
			")or die(mysql_error());
			...
			if ($NUM_VM_ORDER_DETAIL_RESULTS>$HIGHEST_NAME_NUM) {
				$VM_NAME = "Database " . $NUM_VM_ORDER_DETAIL_RESULTS;
			} else { 
				$VM_NAME = "Database " . $HIGHEST_NAME_NUM;
			}
			...
			mysql_query("UPDATE VM_ORDER SET QTY_DATABASE ='$NUM_VM_ORDER_DETAIL_RESULTS' WHERE VM_ORDER_ID='$VM_ORDER_ID'");
			echo '	<form name="Next" action="index.php" method="post" style="display:inline; margin:0; padding:0;">
