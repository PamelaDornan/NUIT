							...
							if ( $wait<>0 and $userquit==1) then
								...
								If (($wait == "Windows Security" or $wait == "Connect to") and $userquit==1) Then
									...
									If $available = 0 AND $available2 = 0  Then
										WinSetState($GUIWindowID,"",@SW_RESTORE)
										max()
										OutlookSetupNotOpen()
									Else
										...
										ElseIf $wait == "Windows Security" Then
											...
											ControlSend( "[CLASS:#32770;TITLE:Windows Security]", "","","{DOWN}")
											ControlSetText ( "[CLASS:#32770;TITLE:Windows Security]", "", "[CLASS:Edit; INSTANCE:4]", "ADS\")
										EndIf
										...
										do
											$userquit = WinExists("[CLASS:#32770;TITLE:Add New]","")
											$wait = WinExists("[CLASS:#32770;TITLE:Add New]", "is successfully configured")
											$x=$x+1
										Until ($wait<>0 or $x>80000 or $userquit==0)
										if ( $wait<>0 and $userquit==1) then 
											sleep(1000)
											ControlSend ( "[CLASS:#32770;TITLE:Add New]", "", "", "{ENTER}")
											sleep(1000)
											ControlSend ( "[CLASS:#32770;TITLE:Outlook]", "", "", "{ENTER}")
											sleep(1000)
											ControlSend ( "[CLASS:#32770;TITLE:Add New]", "", "", "{ESC}")
											ControlSend ( "[CLASS:#32770;TITLE:Account Settings]", "", "", "{ESC}")
											ControlSend ( "[CLASS:#32770;TITLE:Mail Setup]", "", "", "{ESC}")
