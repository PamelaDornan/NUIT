...
; #FUNCTIONS#  ===================================================================================================================
	...
	;If data passes validation it will pass data onto function ConfigureOutlook.
		Func ValidateOutlook()
			...
			EndIf
			If $flag=0 Then
				OutlookAccountDuplicate()
			Else
				OutlookInformationDisplay()
			EndIf
		EndFunc
		
		Func OutlookAccountDuplicate()
			...
			;Next
			;If $sProp == "" Then
			;	$PID1 = ProcessExists("OSPPSVC.EXE") ; Will return the PID or 0 if the process isn't found.
			;	If $PID1 Then ProcessClose("OSPPSVC.EXE")
			;	$PID2 = ProcessExists("OUTLOOK.EXE") ; Will return the PID or 0 if the process isn't found.
			;	If $PID2 Then ProcessClose("OUTLOOK.EXE")
				ConfigureOutlook()
			;else
			;	OutlookExists()
			;endif
	...
	;Function configures Outlook account by using control panel's Mail configuration set.	
							...
							Else
								...
								Else
									...
									If $wait == "Connect to" Then
										...
										WinWait ( "[CLASS:#32770;TITLE:Connect to]","",10)
									EndIf
									If $wait == "Windows Security" and  $OSversion == "WIN_VISTA" Then
										ControlSetText ( "[CLASS:#32770;TITLE:Windows Security]", "", "[CLASS:Edit; INSTANCE:1]", "ADS\" & $netid)
										ControlSetText ( "[CLASS:#32770;TITLE:Windows Security]", "", "[CLASS:Edit; INSTANCE:2]", $password)
										ControlClick ( "[CLASS:#32770;TITLE:Windows Security]", "", "[CLASS:Button; INSTANCE:1]")
										WinWait ( "[CLASS:#32770;TITLE:Windows Security]","",10)
		...
		Func NoProfile()
			...
			Global $X = ((@DesktopWidth-25)/2) - 200		
			Global $Y = ((@DesktopHeight-100)/2) - 200	
			Global $LabelID = GUICtrlCreateLabel("No Microsoft Outlook profiles have been created. Before running the this tool again," & @CRLF & "please click the Start button, and then navigate to Control Panel > Mail.  Click Show" & @CRLF & "Profiles, and then click Add.  Also be sure radio Allways Use This Profile is selected.", $X-60, $Y+30)
			Global $ExitID = GuiCtrlCreateButton("Cancel", $X+400, $Y+250, 80, 30,$BS_FLAT)
			GUICtrlSetOnEvent(-1,"OnExit")
			WindowSuperKill()
		EndFunc
		...
		Func ProfilePrompting()
			...
			Global $X = ((@DesktopWidth-25)/2) - 200		
			Global $Y = ((@DesktopHeight-100)/2) - 200	
			Global $LabelID = GUICtrlCreateLabel("No Microsoft Outlook profiles have been created.  Before running the this tool again," & @CRLF & "please click the Start button, and then navigate to Control Panel > Mail. Click Show" & @CRLF & "Profiles, and then select radio Allways Use This Profile.", $X-60, $Y+30)
			Global $ExitID = GuiCtrlCreateButton("Cancel", $X+400, $Y+250, 80, 30,$BS_FLAT)
			GUICtrlSetOnEvent(-1,"OnExit")
			WindowSuperKill()
		EndFunc
		...
		Func OutlookFinished()
			...
			Global $X = ((@DesktopWidth-25)/2) - 200	
			Global $Y = ((@DesktopHeight-100)/2) - 200	
			Global $LabelID = GUICtrlCreateLabel("Access to your Northwestern Collaboration Services account within Outlook has been" & @CRLF & "successfully added. You may now click Close to log into Outlook.  Note that you may" & @CRLF & "need to re-enter your NetID password to begin.", $X-60, $Y+30)
			Global $LabelID2 = GUICtrlCreateLabel("User Name: ADS\" & $netid, $X-60, $Y+100)
