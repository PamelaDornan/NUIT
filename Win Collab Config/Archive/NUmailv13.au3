...
; #FUNCTIONS#  ===================================================================================================================
	...
	;Function configures Outlook account by using control panel's Mail configuration set.	
							...
							if ( $wait<>0 and $userquit==1) then
								...
								If (($wait == "Windows Security" or $wait == "Connect to") and $userquit==1) Then
									...
									Else
										...
										MsgBox ( 262144, "NUConfig","Please click Finish.")
										$wait = "1"
										$x=1
										do
											$wait = WinExists("[CLASS:#32770;TITLE:Add New]", "is successfully configured")
											$x=$x+1
										Until ($wait<>1 or $x>80000 or $userquit==0)
										if ( $wait<>1) then 
		...
		Func OutlookFinished()
			...
			Global $X = ((@DesktopWidth-25)/2) - 200	
			Global $Y = ((@DesktopHeight-100)/2) - 200	
			Global $LabelID = GUICtrlCreateLabel("Access to your Northwestern Collaboration Services account within Outlook has been" & @CRLF & "successfully added. You may now click Close and log into Outlook.  Note that you may" & @CRLF & "need to re-enter your NetID password to begin.", $X-60, $Y+30)
